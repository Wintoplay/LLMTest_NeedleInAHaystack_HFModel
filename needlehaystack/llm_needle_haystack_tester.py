...
class LLMNeedleHaystackTester:
        ...
        if context_lengths is None:
            if context_lengths_min is None or context_lengths_max is None or context_lengths_num_intervals is None:
                ...
                self.context_lengths = np.round(
                    np.linspace(context_lengths_min,
                                context_lengths_max,
        ...
        if document_depth_percents is None:
            ...
            if document_depth_percent_interval_type == 'linear':
                self.document_depth_percents = np.round(
                    np.linspace(document_depth_percent_min,
                                document_depth_percent_max,
                                num=document_depth_percent_intervals,
                                endpoint=True)).astype(int)
            elif document_depth_percent_interval_type == 'sigmoid':
                self.document_depth_percents = [
                    self.logistic(x) for x in np.linspace(document_depth_percent_min, document_depth_percent_max,
    ...
    async def evaluate_and_log(self, context_length, depth_percent):
        ...
        context_file_location = f'{self.model_name.replace(".", "_")}_{self.difficulty}_len_{context_length}_depth_{int(depth_percent*100)}'
    ...
    def insert_needle(self, context, depth_percent, context_length):
        match self.difficulty:
            case "easy":
                tokens_needle = self.model_to_test.encode_text_to_tokens(f'{self.retrieval_question}\n{self.needle}')
            case "normal":
        ...
        if depth_percent == 100:
            ...
            period_tokens.append(self.model_to_test.encode_text_to_tokens('day.')[-1])
            period_tokens = set(period_tokens)
